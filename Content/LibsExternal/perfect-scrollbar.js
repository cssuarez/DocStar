...
(function (factory) {
    ...
    var defaultSettings = {
        ...
        childElementsToScroll: ['.ui-jqgrid-bdiv', 'select', 'ul.children']
    };
        ...
        return this.each(function () {
            ...
            var updateScrollbarCss = function () {
                if (!settings.notInContainer) {
                    ...
                    $scrollbarXRail.css({ bottom: scrollbarXBottom - $this.scrollTop(), width: containerWidth });
                    $scrollbarYRail.css({ right: scrollbarYRight - $this.scrollLeft(), height: containerHeight });
                }
            ...
            var updateBarSizeAndPosition = function () {
                ...
                if (settings.notInContainer) {
                    $scrollbarXRail.css('position', 'fixed');
                    $scrollbarYRail.css('position', 'fixed');
                    $scrollbarXRail.css({ top: $this.offset().top + $this.height() });
                    $scrollbarYRail.css({ left: $this.offset().left + $this.width(), top: $this.offset().top });
                }
            ...
            var childElementIsScrollable = function (e) {
                var elems = settings.childElementsToScroll;
                ...
                var i = 0;
                for (i = 0; i < length; i++) {
                    $scrollElem = $(elems[i]).has($targ);
                    var elemExists = $scrollElem && $scrollElem.length > 0;
                    if (elemExists) {
                        var scrollHeight = $scrollElem.get(0).scrollHeight;
                        if ($scrollElem.is('select')) {
                            scrollHeight = $scrollElem.find('option').height() * $scrollElem.find('option').length;
                        }
                        var hasOverflow = elemExists && scrollHeight > $scrollElem.outerHeight();
                        if (hasOverflow) {
                            return true;
                        }
                    }
            ...
            var bindMouseWheelHandler = function () {
                ...
                $this.bind('mousewheel.perfect-scrollbar-' + UID, function (e, delta, deltaX, deltaY) {
                    ...
                    Utility.updateComboboxDropdownPosition();
            ...
            var bindRailClickHandler = function () {
                ...
                $scrollbarY.bind('click.perfect-scrollbar-' + UID, stopPropagation);
                $scrollbarYRail.bind('click.perfect-scrollbar-' + UID, function (e) {
                ...
                $scrollbarX.bind('click.perfect-scrollbar-' + UID, stopPropagation);
                $scrollbarXRail.bind('click.perfect-scrollbar-' + UID, function (e) {
