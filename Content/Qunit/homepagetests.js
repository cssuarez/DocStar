var HomePageTests = {
	init: function() {
		module('HomePage Tests');
		test('Home Tab exists', function() {
			ok(Base.h.$('a[href="#Home"]').text() === 'Home', 'home exists');
		});
		test('Home Tab is visible', function() {
			ok(Base.h.$('#home_tab').is(':visible'), 'testing if this panel is showing');
		});
		Base.h.$('.ptitle').mouseover();
		Base.sleep(4);
		test('Menu tests', function() {
			ok(Base.h.$('#requestHelp').text() === "Email Support", 'Email Support exists');
