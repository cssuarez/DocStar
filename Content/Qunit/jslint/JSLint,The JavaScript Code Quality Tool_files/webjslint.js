...
if(nx.test(this)){return'"'+this.replace(nxg,function(a){if(escapes[a]){return escapes[a];}
...
the_token=Object.create(syntax[(type==='(punctuator)'||(type==='(identifier)'&&Object.prototype.hasOwnProperty.call(syntax,value))?value:type)]||syntax['(error)']);if(type==='(identifier)'){the_token.identifier=true;if(value==='__iterator__'||value==='__proto__'){fail_at('reserved_a',line,from,value);}else if(option.nomen&&(value.charAt(0)==='_'||value.charAt(value.length-1)==='_')){warn_at('dangling_a',line,from,value);}}
...
line=0;next_line();from=1;},range:function(begin,end){var c,value='';from=character;if(source_row.charAt(0)!==begin){fail_at('expected_a_b',line,character,begin,source_row.charAt(0));}
for(;;){source_row=source_row.slice(1);character+=1;c=source_row.charAt(0);switch(c){case'':fail_at('missing_a',line,character,c);break;case end:source_row=source_row.slice(1);character+=1;return it('(range)',value);case xquote:case'\\':warn_at('unexpected_a',line,character,c);break;}
...
if(source_row){if(xmode==='html'){return it('(error)',source_row.charAt(0));}else{fail_at('unexpected_a',line,character,source_row.substr(0,1));}}}else{if(c.isAlpha()||c==='_'||c==='$'){return it('(identifier)',symbol);}
...
collect_comment(source_row,quote,line,character);quote='';if(!next_line()){fail_at('unclosed_comment',line,character);}}
collect_comment(source_row.slice(0,i),quote,character,line);character+=i+2;if(source_row.substr(i,1)==='/'){fail_at('nested_comment',line,character);}
source_row=source_row.substr(i+2);break;case'':break;case'/':if(token.id==='/='){fail_at(bundle.slash_equal,line,from);}
if(prereg){depth=0;captures=0;length=0;for(;;){b=true;c=source_row.charAt(length);length+=1;switch(c){case'':fail_at('unclosed_regexp',line,from);return;case'/':if(depth>0){warn_at('unescaped_a',line,from+length,'/');}
...
if(source_row.charAt(length).isAlpha()){fail_at('unexpected_a',line,from,source_row.charAt(length));}
character+=length;source_row=source_row.substr(length);quote=source_row.charAt(0);if(quote==='/'||quote==='*'){fail_at('confusing_regexp',line,from);}
...
break;case'[':c=source_row.charAt(length);if(c==='^'){length+=1;if(option.regexp){warn_at('insecure_a',line,from+length,c);}else if(source_row.charAt(length)===']'){fail_at('unescaped_a',line,from+length,'^');}}
...
i=source_row.indexOf('<!');if(i>=0){fail_at('nested_comment',line,character+i);}
if(!next_line()){fail_at('unclosed_comment',length,c);}}
length=source_row.indexOf('<!');if(length>=0&&length<i){fail_at('nested_comment',line,character+length);}
character+=i;if(source_row.charAt(i+2)!=='>'){fail_at('expected_a',line,character,'-->');}
...
if(!((c>='0'&&c<='9')||(c>='a'&&c<='z')||c==='#')){fail_at('bad_entity',line,from+length,character);}}
break;}
...
dent.open=false;}
var_mode=false;}
if(indent.open){if(next_token.edge){if(next_token.edge==='label'){expected_at(1);}else if(next_token.edge==='case'){expected_at(indent.at-option.indent);}else if(indent.mode!=='array'||next_token.line!==token.line){expected_at(indent.at);}}else if(next_token.line!==token.line&&next_token.from<indent.at+(indent.mode==='expression'?0:option.indent)){expected_at(indent.at+option.indent);}}else if(next_token.line!==token.line){if(next_token.edge){expected_at(indent.at);}else{indent.wrap=true;if(indent.mode==='statement'||indent.mode==='var'){expected_at(indent.at+option.indent);}else if(next_token.from<indent.at+(indent.mode==='expression'?0:option.indent)){expected_at(indent.at+option.indent);}}}}
...
function directive(){var command=this.id,name,old_comments_off=comments_off,old_option_white=option.white,value;comments_off=true;option.white=false;if(lookahead.length>0||next_token.comments){warn('unexpected_a',this);}
...
break;case'/*global':if(option.safe){warn('adsafe_a',this);}
break;default:fail('unpexpected_a',this);}
loop:for(;;){for(;;){if(next_token.id==='*/'){break loop;}
if(next_token.id!==','){break;}
advance();}
if(next_token.arity!=='string'&&!next_token.identifier){fail('unexpected_a',next_token);}
name=next_token.value;advance();switch(command){case'/*global':if(next_token.id===':'){advance(':');switch(next_token.id){case'true':if(typeof scope[name]==='object'||global[name]===false){fail('unexpected_a');}
global[name]=true;advance('true');break;case'false':if(typeof scope[name]==='object'){fail('unexpected_a');}
global[name]=false;advance('false');break;default:fail('unexpected_a');}}else{if(typeof scope[name]==='object'){fail('unexpected_a');}
global[name]=false;}
break;case'/*jslint':if(next_token.id!==':'){fail('expected_a_b',next_token,':',next_token.value);}
advance(':');switch(name){case'indent':value=+next_token.value;if(typeof value!=='number'||!isFinite(value)||value<0||Math.floor(value)!==value){fail('expected_small_a');}
if(value>0){old_option_white=true;}
option.indent=value;break;case'maxerr':value=+next_token.value;if(typeof value!=='number'||!isFinite(value)||value<=0||Math.floor(value)!==value){fail('expected_small_a',next_token);}
option.maxerr=value;break;case'maxlen':value=+next_token.value;if(typeof value!=='number'||!isFinite(value)||value<0||Math.floor(value)!==value){fail('expected_small_a');}
option.maxlen=value;break;case'white':if(next_token.id==='true'){old_option_white=true;}else if(next_token.id==='false'){old_option_white=false;}else{fail('unexpected_a');}
...
return false;}
function expression(rbp,initial){var left;if(next_token.id==='(end)'){fail('unexpected_a',token,next_token.id);}
...
if(initial===true&&token.fud){left=token.fud();}else{if(token.nud){left=token.nud();}else{if(next_token.arity==='number'&&token.id==='.'){warn('leading_decimal_a',token,next_token.value);advance();return token;}else{fail('expected_identifier_a',token,token.id);}}
while(rbp<next_token.lbp){advance();if(token.led){left=token.led(left);}else{fail('expected_operator_a',token,token.id);}}}
return left;}
...
that.second=expression(19);if(that.id==='='&&are_similar(that.first,that.second)){warn('weird_assignment',that);}
return that;}}
fail('bad_assignment',that);});x.assign=true;return x;}
...
function suffix(s,f){var x=symbol(s,150);x.led=function(left){no_space_only(prev_token,token);if(option.plusplus){warn('unexpected_a',this);}else if((!left.identifier||left.reserved)&&left.id!=='.'&&left.id!=='['){warn('bad_operand',this);}
...
if(token.id==='function'&&next_token.id==='('){warn('name_function');}else{fail('expected_identifier_a');}}
function statement(no_indent){var label,old_scope=scope,the_statement;if(next_token.id===';'){warn('unexpected_a');semicolon();return;}
...
array=statements();strict_mode=old_strict_mode;step_out('}',curly);discard();}else if(!ordinary){fail('expected_a_b',next_token,'{',next_token.value);}else{warn('expected_a_b',next_token,'{',next_token.value);array=[statement()];array.disrupt=array[0].disrupt;}
funct['(verb)']=null;scope=old_scope;in_block=old_inblock;if(ordinary&&array.length===0){warn('empty_block');}
return array;}
function tally_property(name){if(properties&&typeof properties[name]!=='boolean'){warn('unexpected_member_a',token,name);}
...
expression(150);return this;});prefix('!',function(){no_space_only();this.first=expression(150);this.arity='prefix';if(bang[this.first.id]===true){warn('confusing_a',this);}
...
var p=[];if(left){if(left.identifier){if(left.value.match(/^[A-Z]([A-Z0-9_$]*[a-z][A-Za-z0-9_$]*)?$/)){if(left.value!=='Number'&&left.value!=='String'&&left.value!=='Boolean'&&left.value!=='Date'){if(left.value==='Math'||left.value==='JSON'){warn('not_a_function',left);}else if(left.value==='Object'){warn('use_object',token);}else if(left.value==='Array'||option.newcap){warn('missing_a',left,'new');}}}}else if(left.id==='.'){if(option.safe&&left.first.value==='Math'&&left.second==='random'){warn('adsafe',left);}}}
...
that.first=left;that.second=token;if(left&&left.value==='arguments'&&(name==='callee'||name==='caller')){warn('avoid_a',left,'arguments.'+name);}else if(!option.evil&&left&&left.value==='document'&&(name==='write'||name==='writeln')){warn('write_is_wrong',left);}else if(option.adsafe){if(!adsafe_top&&left.value==='ADSAFE'){if(name==='id'||name==='lib'){warn('adsafe',that);}else if(name==='go'){if(xmode!=='script'){warn('adsafe',that);}else if(adsafe_went||next_token.id!=='('||peek(0).arity!=='string'||peek(0).value!==adsafe_id||peek(1).id!==','){fail('adsafe_a',that,'go');}
...
return that;},true);infix('[',170,function(left,that){no_space_only(prev_token,token);no_space();step_in();edge();var e=expression(0),s;if(e.arity==='string'){if(option.safe&&banned[e.value]===true){warn('adsafe_a',e);}else if(!option.evil&&(e.value==='eval'||e.value==='execScript')){warn('evil',e);}else if(option.safe&&(e.value.charAt(0)==='_'||e.value.charAt(0)==='-')){warn('adsafe_subscript_a',e);}
tally_property(e.value);if(!option.sub&&ix.test(e.value)){s=syntax[e.value];if(!s||!s.reserved){warn('subscript',e);}}}else if(e.arity!=='number'||e.value<0){if(option.safe){warn('adsafe_subscript_a',e);}}
...
if(next_token.id===']'){break;}
edge();this.first.push(expression(10));if(next_token.id===','){comma();if(next_token.id===']'&&!option.es5){warn('unexpected_a',token);break;}}else{break;}}
...
for(;;){edge();id=identifier();params.push(token);add_label(id,'parameter');if(next_token.id===','){comma();}else{no_space();step_out(')',paren);discard();return params;}}}
function do_function(func,name){var old_properties=properties,old_option=option,old_global=global,old_scope=scope;funct={'(name)':name||'"'+anonname+'"','(line)':next_token.line,'(context)':funct,'(breakage)':0,'(loopage)':0,'(scope)':scope,'(token)':func};properties=old_properties&&Object.create(old_properties);option=Object.create(old_option);global=Object.create(old_global);scope=Object.create(old_scope);token.funct=funct;functions.push(funct);if(name){add_label(name,'function');}
...
prefix('{',function(){var get,i,j,name,p,set,seen={};this.arity='prefix';this.first=[];step_in();while(next_token.id!=='}'){edge();if(next_token.value==='get'&&peek().id!==':'){if(!option.es5){warn('get_set');}
get=next_token;advance('get');one_space_only();name=next_token;i=property_name();if(!i){fail('missing_property');}
do_function(get,'');if(funct['(loopage)']){warn('function_loop',get);}
p=get.first;if(p){warn('parameter_a_get_b',p[0],p[0].value,i);}
comma();set=next_token;spaces();edge();advance('set');one_space_only();j=property_name();if(i!==j){fail('expected_a_b',token,i,j||next_token.value);}
do_function(set,'');p=set.first;if(!p||p.length!==1){fail('parameter_set_a',set,'value');}else if(p[0].value!=='value'){fail('expected_a_b',p[0],'value',p[0].value);}
name.first=[get,set];}else{name=next_token;i=property_name();if(typeof i!=='string'){fail('missing_property');}
...
if(peek(0).id==='='&&next_token.identifier){fail('var_a_not');}
...
comma();if(var_mode&&next_token.line===token.line&&this.first.length===1){var_mode=false;indent.open=false;indent.at-=option.indent;}
spaces();edge();}
...
if(next_token.id==='finally'){discard();one_space();advance('finally');discard();one_space();this.third=block(false);}else if(!this.second){fail('expected_a_b',next_token,'catch',next_token.value);}
...
one_space();advance('while');discard();var paren=next_token;one_space();advance('(');step_in();discard();no_space();edge();this.first=expected_condition(expected_relation(expression(0)),bundle.unexpected_a);no_space();step_out(')',paren);discard();funct['(breakage)']-=1;funct['(loopage)']-=1;return this;});labeled_stmt('for',function(){var blok,filter,ok=false,paren=next_token,the_in,value;this.arity='statement';funct['(breakage)']+=1;funct['(loopage)']+=1;advance('(');step_in('control');discard();spaces(this,paren);no_space();if(next_token.id==='var'){fail('move_var');}
edge();if(peek(0).id==='in'){value=next_token;switch(funct[value.value]){case'unused':funct[value.value]='var';break;case'var':break;default:warn('bad_in_a',value);}
...
semicolon(token);if(next_token.id===';'){fail('expected_a_b',next_token,')',';');}
...
comma();if(next_token.id===']'){warn('unexpected_a',token);break;}}}
advance(']',bracket);}
switch(next_token.id){case'{':json_object();break;case'[':json_array();break;case'true':case'false':case'null':case'(number)':case'(string)':advance();break;case'-':advance('-');no_space_only();advance('(number)');break;default:fail('unexpected_a');}}
...
if(option.safe&&ux.test(url)){fail('adsafe_a',next_token,url);}
...
advance();}
advance(']');break;default:fail('expected_selector_a');}}}
...
semicolon();break;case'media':advance();for(;;){if(!next_token.identifier||css_media[next_token.value]===true){fail('expected_media_a');}
advance();if(next_token.id!==','){break;}
comma();}
advance('{');style_list();advance('}');break;default:warn('expected_at_a');}}else{warn('expected_at_a');}}
style_list();}
function do_begin(n){if(n!=='html'&&!option.fragment){if(n==='div'&&option.adsafe){fail('adsafe_fragment');}else{fail('expected_a_b',token,'html',n);}}
if(option.adsafe){if(n==='html'){fail('adsafe_html',token);}
if(option.fragment){if(n!=='div'){fail('adsafe_div',token);}}else{fail('adsafe_fragment',token);}}
option.browser=true;assume();}
function do_attribute(n,a,v){var u,x;if(a==='id'){u=typeof v==='string'?v.toUpperCase():'';if(ids[u]===true){warn('duplicate_a',next_token,v);}
...
ids[u]=true;}else if(a==='href'||a==='background'||a==='content'||a==='data'||a.indexOf('src')>=0||a.indexOf('url')>=0){if(option.safe&&ux.test(v)){fail('bad_url',next_token,v);}
...
function do_tag(name,attribute){var i,tag=html_tag[name],script,x;src=false;if(!tag){fail(bundle.unrecognized_tag_a,next_token,name===name.toLowerCase()?name:name+' (capitalization error)');}
if(stack.length>0){if(name==='html'){fail('unexpected_a',token,name);}
x=tag.parent;if(x){if(x.indexOf(' '+stack[stack.length-1].name+' ')<0){fail('tag_a_in_b',token,name,x);}}else if(!option.adsafe&&!option.fragment){i=stack.length;do{if(i<=0){fail('tag_a_in_b',token,name,'body');}
...
if(attribute.type){warn('type',token);}}else{step_in(next_token.from);edge();use_strict();adsafe_top=true;script=statements();if(option.adsafe){if(adsafe_went){fail('adsafe_script',token);}
if(script.length!==1||aint(script[0],'id','(')||aint(script[0].first,'id','.')||aint(script[0].first.first,'value','ADSAFE')||aint(script[0].second[0],'value',adsafe_id)){fail('adsafe_id_go');}
switch(script[0].first.second.value){case'id':if(adsafe_may||script[0].second.length!==1){fail('adsafe_id',next_token);}
adsafe_may=true;break;case'go':if(!adsafe_may){fail('adsafe_id');}
if(script[0].second.length!==2||aint(script[0].second[1],'id','function')||script[0].second[1].first.length!==2||aint(script[0].second[1].first[0],'value','dom')||aint(script[0].second[1].first[1],'value','lib')){fail('adsafe_go',next_token);}
adsafe_went=true;break;default:fail('adsafe_id_go');}}
indent=null;}
...
tag=html_tag[name];if(typeof tag!=='object'){fail('unrecognized_tag_a',tag_name,name);}
...
xmode='scriptstring';advance('=');quote=next_token.id;if(quote!=='"'&&quote!=='\''){fail('expected_a_b',next_token,'"',next_token.value);}
xquote=quote;wmode=option.white;option.white=false;advance(quote);use_strict();statements();option.white=wmode;if(next_token.id!==quote){fail('expected_a_b',next_token,quote,next_token.value);}
xmode='html';xquote='';advance(quote);tag=false;}else if(attribute==='style'){xmode='scriptstring';advance('=');quote=next_token.id;if(quote!=='"'&&quote!=='\''){fail('expected_a_b',next_token,'"',next_token.value);}
...
attributes[attribute]=tag;do_attribute(name,attribute,tag);}
...
advance();if(!stack){fail('unexpected_a',next_token,closetag(name));}
tag_name=stack.pop();if(!tag_name){fail('unexpected_a',next_token,closetag(name));}
if(tag_name.name!==name){fail('expected_a_b',next_token,closetag(tag_name.name),closetag(name));}
if(next_token.id!=='>'){fail('expected_a_b',next_token,'>',next_token.value);}
...
if(next_token.value.indexOf('--')>=0){fail('unexpected_a',next_token,'--');}
if(next_token.value.indexOf('<')>=0){fail('unexpected_a',next_token,'<');}
if(next_token.value.indexOf('>')>=0){fail('unexpected_a',next_token,'>');}}
xmode='outer';advance('>');break;case'(end)':return;default:if(next_token.id==='(end)'){fail('missing_a',next_token,'</'+stack[stack.length-1].value+'>');}else{advance();}}
if(stack&&stack.length===0&&(option.adsafe||!option.fragment||next_token.id==='(end)')){break;}}
if(next_token.id!=='(end)'){fail('unexpected_a');}}
